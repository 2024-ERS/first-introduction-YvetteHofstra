...
# database: (remove hashtag) goes to a new tab (internet)
...
dat_month |> ggplot(aes(x=mdate,y=avgtemp_mo_oC)) +
  geom_line()
# maximum has not changed much, but 2011 was last time whole month was -
# But the minimum is softer
# Cold winter -> strong cohort of cockles, crabs don't leave the gulleys 
# so cockles on mudflats are safe from being eaten 

dat_month |> ggplot(aes(x=mdate,y=avgtemp_mo_oC)) +
  geom_line() +
  tidyquant::geom_ma(ma_fun=SMA,n=61,col="red",linetype="solid") +
  coord_x_date(xlim=c("2014-01-01","2024-08-31")) + # zoom to this part
...
#  tidyquant::geom_ma(ma_fun=SMA,n=13,zoo::align="right",
...
# additive time series decomposition of the monthly mean temperatures: what is seasonal and what is a long term trend?
...
# sum of all avg negative daily temps, sum of freezing temps in winter
dat_Hellman<-dat |>
...
tail(dat_Hellman,10)
# 2017 f.e. 3 days: -10 -10 and -1 together makes -21
...
dat_WarmthIndex # left join to read and add the data of cokcles

# How to add quantile regressions? So how many % of the points is above or below the line. Previous example ~ 50%-50%
dat_WarmthIndex |>
  ggplot(aes(x=year,y=WarmthIndex)) +
  geom_point(size=2) +
  geom_quantile(quantiles = c(0.1, 0.5, 0.9), 
                col="purple",linetype="dashed", linewidth= 1) +
  labs(title="Warmth Index Over Time with Quantile Regressions") +
  geom_smooth(method="lm",col="red")
# Warm years not getting warmer
# Cold years are getting warmer
# In dessert even nights getting warmer rather than days getting hotter
# Ectotherms (f.e. fish) are affected. In water can stay active longer
# Also getting wetter (cannot see with this data)
