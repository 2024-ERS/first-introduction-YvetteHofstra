...
remove(list=ls())
# restore your libraries from the lock file
renv::restore() 
library(tidyverse) 
...
print(orchdat)  # already see what data you work with
...
# make year a factor
names(orchdat)  # to see what they have in common
names(elevdat)  # what do you want to join in 1 file?
orchdat3<-left_join(orchdat,elevdat,by=c("year", "TransectPoint_ID")) |>
  dplyr::filter(TransectPoint_ID>=200 & TransectPoint_ID<=1000) |> 
  dplyr::filter(!(year==2022 & TransectPoint_ID==260)) # get all except this one
...
orchdat3 |>
  ...
  facet_grid(year~.)  # what is in rows vs what is in columns 
...
# calculate new variable (adds prediction for year and elevation)
...
# y=b0 + b1x1 + b2x1^2   negative b2 (second order polynomial) = ecologically logical so expect b2 to be negative 
# y=b0 + b1x1 + b2*I(x^2) 
...
anova(m1,m2)
# residual sum of squares is lower in m2, good so is better
# sum of differences of predicted vs found, higher is model deviates more so less fitting
# also seen that the difference in m1 and m2 is significantly different
...
anova(m3,m2) # yes this is a better model
...
#anova(m4) # degrees of freedom = values -1 (6 years so 5 df)
#anova(m4,m3)  # not significantly improved, so preferred = m3!
...
#  using  multiple regression, assuming a poisson distribution (so use a generalized linear model)
...
anova(m7,m6,test="Chisq") # m7 is much better
 
#add the  model to the plot
# calculate the predicted value of m2 for every observation, add to the data set as a variable as pred2
# add the new predicted line to the previous plot p2, store as object p3 and show it
...
anova(m8,m7,test="Chisq") # m8 improved significantly so = best model
