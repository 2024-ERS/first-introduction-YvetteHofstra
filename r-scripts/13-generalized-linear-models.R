...
ggplot(aes(x=Treatment,y=Value,color=Block)) + 
  geom_point()
# Good reason to account for blocks -> block 3 always higher
data |>
  ggplot(aes(x=Treatment,y=Value,color=Block)) + 
  geom_jitter(width=0.15) + # add some variation to see red better
...
anova(m1)  # But could have randomly selected other variables
# Not correct: you just treat block as if it is fixed, but it is random
...
# Treatment effect is the same for every block
# Fixed slope model
summary(model1)
...
# different slopes for every block
